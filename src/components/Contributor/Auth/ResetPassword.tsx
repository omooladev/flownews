...
const ResetPassword = (props) => {
  ...
  const { viewPassword, toggleViewPasswordHandler, authReply, onChangeAuthReply, onResetAuthReply, onValidateEmail, onValidatePassword } = props;
  ...
  const changePasswordHandler = useCallback(
    async (event) => {
      ...
      const { status, error } = await onUpdate_ResetPassword('reset_password', contributorData.contributorId, {
        password,
        confirmPassword,
      });
      if (status === 200) {
      ...
      {!passwordChanged && (
        <>
          ...
          {isLoading.type !== 'link' && !linkIsValid && linkIsValid !== null && (
            <ForgotPassword linkIsValid={linkIsValid} authReply={authReply} onChangeAuthReply={onChangeAuthReply} onResetAuthReply={onResetAuthReply} onValidateEmail={onValidateEmail} />
          )}
