...
const AuthContextProvider = (props) => {
  ...
  const getContributorData = useCallback(
    async (username) => {
      ...
      if (contributorData.username && (!username || contributorData.username === username || searchedContributorData.username === username)) {
  ...
  const checkFieldExistence = useCallback(
    async ({ name, value }) => {
      ...
      try {
        const response = await sendRequest(`${HOSTURI}/contributor/check-field-existence?fieldName=${name}&fieldValue=${value}`, {
          method: 'POST',
          token,
        });
        const data = response.data;
  ...
  const toggleFollowContributor = useCallback(
    async ({ action }) => {
      ...
      const response = await sendRequest(`${HOSTURI}/contributor/follow?action=${action}&username=${searchedContributorData.username}`, {
        method: 'POST',
        token,
      });
      const data = response.data;
  ...
  const changeProfilePicture = useCallback(
    async ({ action, isCropped, profilePicture }) => {
      ...
      const response = await sendRequest(`${HOSTURI}/contributor/update-profile?action=profile-picture&&actionType=${action}`, {
        method: 'PATCH',
        token,
        contributorData,
        contentType: action === 'save' && 'multipart/form-data',
      });
      return response;
    },
    [token, sendRequest],
  ...
  useEffect(() => {
    ...
    if (!history.location.pathname.startsWith(`/story/${newStory.storyId}/edit`) && !history.location.pathname.startsWith('/new-story') && newStory.storyId !== null) {
  ...
  useEffect(() => {
    if (history.location.pathname.startsWith('/new-story') && newStory.storyId !== null && (newStory.title || newStory.coverImage || newStory.value)) {
  ...
  useEffect(() => {
    const handleBackgroundColor = () => {
      if ((history.location.pathname.includes('/story/draft/preview') || history.location.pathname === `/@${contributorData.username}`) && appMode.theme === 'dark-default') {
