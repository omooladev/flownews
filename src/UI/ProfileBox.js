...
const ProfileBox = ({ options, className, uploadContainerIsActive, onToggleUploadContainer, onClick }) => {
  ...
  let { showIcon = false, showUsername = false, showText = false, showEditButton = false, showYourPersonalAccountText = false } = options || defaultOptions;
    ...
    <section onClick={onClick} className={`${styles.profile_box} ${customClass}`}>
      <div className={styles.profilePicture_Username}>
        {contributorProfilePicture ? (
          <img src={contributorProfilePicture} alt="contributor profile" className={styles.profile_picture} onError={handleProfilePictureError} />
        ) : (
      ...
      {showText && (
        <div className={styles.profile_box_text}>
          ...
          {showYourPersonalAccountText && <h5 className={styles.your_personal_account}>Your personal account</h5>}
        </div>
      )}
      {/* This is the configuration for the profile box that appears on the user form gotten from */}
      {showEditButton && (
        <button onClick={onToggleUploadContainer} className={`${styles.upload_container_button} ${uploadContainerIsActive ? styles.active : ''}`}>
          <FaRegEdit />
          <span>Edit</span>
        </button>
      )}
